...
init([{child_options,#child_options{ restart = _Restart, shutdown = _Shutdown, type = _Type } = HVal}|T], #supervisor_options{ strategy = Strategy, intensity = Intensity, period = Period, child_options = _ChildOptions, statem_options = StatemOptions, child_spec = ChildSpec, child_names = ChildNames } = _Params) -> 
...
init([{statem_options,#statem_options{ allow_delayable_sends = _AllowDelayableSends, printout_enabled = _PrintoutEnabled } = HVal}|T], #supervisor_options{ strategy = Strategy, intensity = Intensity, period = Period, child_options = ChildOptions, statem_options = _StatemOptions, child_spec = ChildSpec, child_names = ChildNames } = _Params) -> 
    Params1 = #supervisor_options{ strategy = Strategy,
                                   intensity = Intensity,
                                   period = Period,
...
init([{child_spec, HVal}|T], #supervisor_options{ strategy = Strategy, intensity = Intensity, period = Period, child_options = ChildOptions, statem_options = StatemOptions, child_spec = ChildSpec, child_names = ChildNames } = _Params) -> 
    % %% remove list of names from ChildSpec
    % Names = maps:get(names, HVal),
    % HVal1 = maps:remove(names, HVal),
    Params1 = #supervisor_options{ strategy = Strategy,
                                   intensity = Intensity,
                                   period = Period,
                                   child_options = ChildOptions,
...
init([], #supervisor_options{ strategy = Strategy, intensity = Intensity, period = Period, child_options = ChildOptions, statem_options = StatemOptions, child_spec = ChildSpec, child_names = _ChildNames } = _Params) -> 
...
run_setup(ChildNames) ->
    printout("~p.", [?FUNCTION_NAME]),

    Children = supervisor:which_children(?MODULE),
    printout("~p, Children: ~p.", [?FUNCTION_NAME, Children]),

    PartyA = lists:nth(1, ChildNames),
    PartyB = lists:nth(2, ChildNames),

    {_, PartyAID, _, _} = lists:last(lists:filter(fun(Child) -> case Child of 
                                                                    {PartyA, _, _, [msg_worker]} -> true;
                                                                    _Else -> false end end, Children)),

    {_, PartyBID, _, _} = lists:last(lists:filter(fun(Child) -> case Child of 
                                                                    {PartyB, _, _, [msg_worker]} -> true;
                                                                    _Else -> false end end, Children)),
    
    printout("~p, ~p: ~p.", [?FUNCTION_NAME, PartyA, PartyAID]),
    printout("~p, ~p: ~p.", [?FUNCTION_NAME, PartyB, PartyBID]),

    PartyAID ! {self(), sup_init, PartyBID},
    PartyBID ! {self(), sup_init, PartyAID},

    %% register pid to name
    register(PartyA, PartyAID),
    register(PartyB, PartyBID),
