...
start(Type, Args) when is_list(Args) -> 
  ...
  %% get params if necessary
  Args1 = maps:from_list(Args),
  Keys = maps:keys(Args1),
  case lists:member(use_preset, Keys) of
    true -> %% if told to use specific params (from file)
      {AppParams, ServerParams} = get_app_params(maps:get(use_preset, Args1)),
      ServerParams1 = ServerParams ++ maps:to_list(maps:remove(use_preset, Args1));
    _ -> %% just use params provided (default_app_params())
      ServerParams1 = Args,
      case lists:member(app_params, Keys) of
        true -> %% if no preset, check for app_params
          AppParams = maps:get(app_params, Keys);
        _ -> %% if none, use default
          AppParams = default_app_params()
      end
  end,

  WorkerNum = maps:get(num_roles, AppParams, ?WORKER_NUM),
  ActivePreset = maps:get(active_preset, AppParams, default),
  ...
  %% add app_params
  ets:insert(tpri, {app_params, AppParams}),
  printout("~p, created ets, added app_params:\n\t~p.", [?FUNCTION_NAME, AppParams]),

  {ok, ServerID} = tpri_server:start_link(ServerParams1),
  printout("~p, server started: ~p.\n", [?FUNCTION_NAME,ServerID]),

  printout("~p, beginning ets setup.", [?FUNCTION_NAME]),

  %% get list of role names
  Roles = lists:filtermap(fun(Elem) -> 
      case Elem of
        {role, #role_spec{name=Name,modules=_Modules,params=_Params}} -> {true, Name};
        _ -> false
      end
  end, ServerParams1),
  printout("~p, roles: ~p.", [?FUNCTION_NAME, Roles]),

  %% for sup tree of mon and imp
  DefaultRolePreset = [sup,mon,imp],
  case ActivePreset of
    asym_direct_imp -> 
      AsymRole = maps:get(role, maps:get(asym_direct_imp, AppParams)),
      RolePresets = lists:foldl(fun(Elem,Acc) -> 
          case Elem=:=AsymRole of
            true -> maps:put(Elem, [imp], Acc);
            _ -> maps:put(Elem, DefaultRolePreset, Acc)
          end
      end, #{}, Roles);
    _ -> 
      RolePresets = lists:foldl(fun(Elem,Acc) -> maps:put(Elem, DefaultRolePreset, Acc) end, #{}, Roles)
  end,
  printout("~p, role presets: ~p.", [?FUNCTION_NAME, RolePresets]),
...
run() ->
  ...
  %% check for any special modes/preset
  AppParams = ets_get(tpri, app_params),
  ActivePreset = maps:get(active_preset, AppParams, default),
  printout("~p, active preset: ~p.", [?FUNCTION_NAME, ActivePreset]),
  case ActivePreset of
    default -> run_startup(ActivePreset, {A, B}, #{});
    _ -> run_startup(ActivePreset, {A, B}, maps:get(ActivePreset, AppParams, #{}))
  end,
  ok.
