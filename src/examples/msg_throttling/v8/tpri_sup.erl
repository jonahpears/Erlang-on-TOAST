...
init(Params) -> 
  ...
  BobTempl = #{ init => state1_recv_msg1,
  ...
  Default = [ {child_options, #child_options{ restart = temporary, % transient
              ...
              {role, #role_spec{ name = ali, modules = #role_modules{mon=role_tmp}, params = [] ++ [{fsm, AliTempl}] }},
              {role, #role_spec{ name = bob, modules = #role_modules{mon=role_tmp}, params = [] ++ [{fsm, BobTempl}] }} ],
